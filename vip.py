...
def getinfo():
    ...
    date = sitreps[0].split('/')[-1].split('_')[2]
    joined = folder + '\\' + ' '.join(name[:-3]) + ' ' + date + '.xlsx'
   
    
    # get vip list no tagging
    dfnotag = dfpast.loc[(dfpast['Star Customer Type'] == 'No'), ['Contract Acc','Star Customer Type']]
    ...
    for i in sitreps:
        ...
        # dfsit.to_csv('see.csv', index = False)
        # print(dfsit['State'])
        ...
        # df = df.loc[(pd.isnull(df['State'] != True))]
        print('-----------')
        # print(df)
        
    # df.to_csv('out.xlsx', index = False)
...
def export():
    global filenames, sitreps, pastpath
    ...
    kv = kv.loc[(df['Installation Type'].astype(str) != '25')]
...
Button(root, text = 'Extract Information', command = getinfo).place(x = 90, y = 170)
Button(root, text = 'Export for Dashboard & Monitoring', command = export).place(x = 205, y = 170)
# Button(root, text = 'Export for Monitoring', command = test).place(x = 320, y = 170)
